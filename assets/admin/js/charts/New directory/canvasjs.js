/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v1.8.1 Beta 1 - http://canvasjs.com/ 
...
(function () {
	...
	Chart.prototype.validateRegion = function (px1, py1, px2, py2, keepAxisIndependent) {
		...
		return { isValid: isValid, axesWithValidRange: axesWithValidRange, axesRanges: axesRanges };
	}
	...
	Chart.prototype.renderIndexLabels = function (targetCtx) {
		...
		for (var i = 0; i < this._indexLabels.length; i++) {
			...
			if (indexLabelLineThickness && placement !== "inside" &&
				(chartTypeLower.indexOf("bar") < 0 && indexLabel.point.x > plotArea.x1 && indexLabel.point.x < plotArea.x2 || indexLabel.point.x - visibleWidth - 2 * marginX - offset > plotArea.x1 && indexLabel.point.x + visibleWidth + 2 * marginX + offset < plotArea.x2) &&
				(chartTypeLower.indexOf("column") < 0 && indexLabel.point.y > plotArea.y1 && indexLabel.point.y < plotArea.y2 || indexLabel.point.y - visibleHeight - 2 * marginY - offset > plotArea.y1 && indexLabel.point.y + visibleHeight + 2 * marginY + offset < plotArea.y2)) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (isFirstDataPointInPlotArea || prevDataNull) {
						...
						//If previous to first dataPoint in the plotArea is null to connect with notNull previous data
						if (isFirstDataPointInPlotArea && i > 0 && prevDataNull && dataSeries.connectNullData) {
							for (var k = i - 1; k >= 0 ; k--) {
								if (typeof (dataPoints[k].y) === "number") {
									isFirstDataPointInPlotArea = false;

									var prevNotNullX = (plotUnit.axisX.conversionParameters.reference + plotUnit.axisX.conversionParameters.pixelPerUnit * ((dataPoints[k].x.getTime ? dataPoints[k].x.getTime() : dataPoints[k].x) - plotUnit.axisX.conversionParameters.minimum) + .5) << 0;
									var prevNotNullY = (plotUnit.axisY.conversionParameters.reference + plotUnit.axisY.conversionParameters.pixelPerUnit * (dataPoints[k].y - plotUnit.axisY.conversionParameters.minimum) + .5) << 0;

									ctx.beginPath();
									ctx.moveTo(prevNotNullX, prevNotNullY);
									if (isCanvasSupported) {
										ghostCtx.beginPath();
										ghostCtx.moveTo(prevNotNullX, prevNotNullY);
									}

									if (dataPoints[k].lineColor && dataPoints[k].lineColor !== currentStrokeStyle || currentLineDashType !== (dataPoints[k].lineDashType || dataSeries.nullDataLineDashType)) {
										currentStrokeStyle = dataPoints[k].lineColor || color;
										ctx.strokeStyle = currentStrokeStyle;
										if (ctx.setLineDash)
											if (dataPoints[k].lineDashType) {
												currentLineDashType = dataPoints[k].lineDashType;
												ctx.setLineDash(getLineDashArray(currentLineDashType, dataSeries.lineThickness));
											}// nullDataLineDashType will be handled in next section
									}
						...
						//For drawing line over notNull dataPoints
						if (!isFirstDataPointInPlotArea && dataSeries.connectNullData) {
							...
							if (ctx.setLineDash && (currentLineDashType === dataSeries.lineDashType && dataSeries.lineDashType !== dataSeries.nullDataLineDashType)) {
								ctx.stroke();
								//if (isCanvasSupported) {
								//	ghostCtx.stroke();
								//}
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (isFirstDataPointInPlotArea || prevDataNull) {
						...
						if (isFirstDataPointInPlotArea && i > 0 && prevDataNull && dataSeries.connectNullData) {
							for (var k = i - 1; k >= 0 ; k--) {
								if (typeof (dataPoints[k].y) === "number") {
									...
									ctx.beginPath();
									ctx.moveTo(prevNotNullX, prevNotNullY);
									ctx.lineTo(x, prevNotNullY);
									if (isCanvasSupported) {
										ghostCtx.beginPath();
										ghostCtx.moveTo(prevNotNullX, prevNotNullY);
										ghostCtx.lineTo(x, prevNotNullY);
									}
									break;
								}
							}
						...
						if (!isFirstDataPointInPlotArea && dataSeries.connectNullData) {
							...
							if (ctx.setLineDash && (currentLineDashType === dataSeries.lineDashType && dataSeries.lineDashType !== dataSeries.nullDataLineDashType)) {
								ctx.stroke();
								//if(!dataPoints[prevNotNullDataPointIndex].lineDashType)
								currentLineDashType = dataSeries.nullDataLineDashType;
								ctx.setLineDash(nullDataLineDashType);
							}
							ctx.lineTo(x, prevY);
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (typeof (dataPoints[i].y) !== "number") {
						if (i > 0 && !prevDataNull /*&& (!dataSeries.connectNullData || dataSeries.nullDataLineDashType !== dataPoints[i-1].lineDashType)*/) {// if first dataPoint is null then no need to call stroke method
							renderBezier(pixels);
							var lastPixel = pixels.length < 1 ? [] : pixels[pixels.length - 1];
							pixels = dataSeries.connectNullData ? [lastPixel] : [];
						}
					...
					if (prevDataNull && dataSeries.connectNullData) {
						if (isFirstDataPointInPlotArea && i > 0) {
							for (var k = i - 1; k >= 0 ; k--) {
								if (typeof (dataPoints[k].y) === "number") {

									pixels[0] = {
										x: (plotUnit.axisX.conversionParameters.reference + plotUnit.axisX.conversionParameters.pixelPerUnit * ((dataPoints[k].x.getTime ? dataPoints[k].x.getTime() : dataPoints[k].x) - plotUnit.axisX.conversionParameters.minimum) + .5) << 0,
										y: (plotUnit.axisY.conversionParameters.reference + plotUnit.axisY.conversionParameters.pixelPerUnit * (dataPoints[k].y - plotUnit.axisY.conversionParameters.minimum) + .5) << 0
									};
									if (dataPoints[k].lineColor && dataPoints[k].lineColor !== currentStrokeStyle || currentLineDashType !== (dataPoints[k].lineDashType || dataSeries.nullDataLineDashType)) {
										currentStrokeStyle = dataPoints[k].lineColor || color;
										ctx.strokeStyle = currentStrokeStyle;
						...
						if (ctx.setLineDash && !isFirstDataPointInPlotArea && (currentLineDashType === dataSeries.lineDashType && dataSeries.lineDashType !== dataSeries.nullDataLineDashType)) {
							ctx.setLineDash(nullDataLineDashType);
	...
	Legend.prototype.render = function () {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			for (var itemIndex = 0; itemIndex < row.items.length; itemIndex++) {
				...
				this.ctx.save();
				this.ctx.rect(left, top, maxWidth, Math.max(maxHeight - (maxHeight % this.lineHeight), top));
				this.ctx.clip();
	...
	Axis.prototype.createLabels = function () {
				...
				if (!(this.chart.panEnabled) && this._labels.length >= 1) {
					...
					for (i = 0; i < this._labels.length; i++) {
						...
						if (!(isNullOrUndefined(this._options.labelAngle) && isNaN(this._options.labelAngle)) || this._options.labelAngle === 0) {//User has set angle -->Rotate
							...
							else if (isNullOrUndefined(this._options.labelWrap)) {//User not set wrap
								if (this.labelWrap && this._options.labelMaxWidth) {//labelwrap->true by default -->Rotate+Wrap
									this.sessionVariables.labelWrap = this.labelWrap;
									this.sessionVariables.labelMaxWidth = this._options.labelMaxWidth ? this._options.labelMaxWidth > this.chart.height * .8 >> 0 ? this.chart.height * .8 >> 0 : this._options.labelMaxWidth : labelMaxWidth;
									this.sessionVariables.labelMaxHeight = labelMaxHeight;
								}
								...
								if (!isNullOrUndefined(this._options.labelMaxWidth)) {//User has set labelMaxWidth -->Wrap if user set labelMaxWidth<labelMaxWidth else Rotate+Wrap
									...
									if (!isNullOrUndefined(sizeNext)) {
										if (((size.width + sizeNext.width) >> 0) >= (2 * labelMaxWidth) && ((size.width + sizeNext.width) >> 0) < (2.4 * labelMaxWidth)) {//Reduce Font size
											...
											this.sessionVariables.labelAngle = -25;
											this.sessionVariables.labelMaxWidth = labelMaxWidth * 2;
											this.sessionVariables.labelFontSize = this.labelFontSize;
										}
		...
		for (var i = 0; i < this.stripLines.length; i++) {
			...
			var backgroundColor = stripLine.labelPlacement === "outside" ? stripLine._options.labelBackgroundColor ? stripLine._options.labelBackgroundColor : "#EEEEEE" : stripLine.startValue ? "#EEEEEE" : stripLine.labelBackgroundColor;
			textBlock = new TextBlock(this.ctx, {
				x: 0,
				y: 0,
				//maxWidth: this.maxHeight,
				...
				fontColor: stripLine._options.labelFontColor || stripLine.labelFontColor,
	...
	Axis.prototype.renderLabelsTicksAndTitle = function () {
		...
		if (this._position === "bottom") {
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (skipLabels && labelCount++ % 2 !== 0 && this.labelAutoFit)
					continue;
	...
	CanvasJS.Chart.version = "v1.8.1 Beta 1";
