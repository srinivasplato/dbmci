...
class Discountscheme_model extends CI_Model {

    private $table='discountscheme';
    ...
    public function all_discounts($pdata, $getcount=null)
    {
        ...
        if($getcount)
        {
            return $this->db->select('id')->from($this->table)->order_by('discount_name','asc')->get()->num_rows();
        }
        else
        {
            $this->db->select('*')->from($this->table)->order_by('discount_name','asc');
        }
