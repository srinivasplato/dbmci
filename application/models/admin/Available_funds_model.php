...
class Available_funds_model extends CI_Model {
    ...
    public function all_transferfunds($pdata, $getcount=null)
    {
        // $res=$this->db->select('*')->from('batchs')->order_by('id','desc')->get()->result_array();
          //echo '<pre>';print_r($res);exit; 
        $columns = array
        ...
        if($getcount)
        {
            ...
            if($pdata['type'] == 'to_transfer_funds'){
            $this->db->where('transfer_funds.to_attachment_id',$pdata['attachment_id']);
             }else if($pdata['type'] == 'from_transfer_funds'){
            $this->db->where('transfer_funds.from_attachment_id',$pdata['attachment_id']);
             }
           // $this->db->where('transfer_funds.created_by',$this->session->userdata('user_id'));
            $this->db->from('transfer_funds');
            return $this->db->order_by('transfer_funds.id','asc')->get()->num_rows();

          //  return $this->db->select('id')->from($this->table)->order_by('category_name','asc')->get()->num_rows();
        }
        else
        {
            $this->db->select('transfer_funds.*');
            if($pdata['type'] == 'to_transfer_funds'){
            $this->db->where('transfer_funds.to_attachment_id',$pdata['attachment_id']);
             }else if($pdata['type'] == 'from_transfer_funds'){
            $this->db->where('transfer_funds.from_attachment_id',$pdata['attachment_id']);
            ...
            $this->db->from('transfer_funds');
            $this->db->order_by('transfer_funds.id','asc');
         // echo $this->db->last_query();exit;
        }
        if(isset($pdata['length']))
        {
