  ...
  <div class="main-content-inner">
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <ul class="breadcrumb">
          ...
          <li class="active">List Payment Modes</li>
        </ul><!-- /.breadcrumb -->            
    </div>
...
<div class="page-content">
  <div class="page-header-1">
      ...
      <?php if( in_array('a',$roleResponsible['payment_modes'])){?>
        
          <a href="<?php echo base_url();?>admin/payment_modes/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add Payment Mode</a>                          
          <input type="hidden" name="hiv" id="hiv" value="0"/>
      
      <?php }?>
      <a href="<?php echo base_url();?>admin/payment_modes/download_payment_modes" class="btn btn-success btn-sm" type="button"><i class="fa fa-download fa-lg"></i> Download Payment modes </a>  
      </div> 
  </div><!-- /.page-header -->
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">                      
                      ...
                      <option value="2">Attachment To</option>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/payment_modes'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
            </table>                                            
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/payment_modes/all_paymentmodes'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Amount Type", "name":"amount_type","orderable": false, "data":"amount_type", "width":"0%" },
            ...
            { "title": "Attached to", "name":"center","orderable": false, "data":"attachment_name", "width":"0%" },
            { "title": "Created On", "name":"created_on","orderable": false, "data":"created_on", "width":"0%" },

          <?php if( in_array('s',$roleResponsible['payment_modes'])){?>
            { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
          <?php }?>

          <?php if( in_array('e',$roleResponsible['payment_modes'])){?>
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          if(aData['amount_type'] == 'expense'){
            var amount_type1 = 'Expense';
            var amount_type2='<p style="color:red">'+amount_type1+'</p>';
          }else{
             var amount_type1 = 'Income';
             var amount_type2='<p style="color:green">'+amount_type1+'</p>';
          }
          

          $(nRow).find('td:eq(1)').html(amount_type2);

          if(aData['status'] == 'Active')
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/payment_modes/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/payment_modes/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
           <?php if( in_array('s',$roleResponsible['payment_modes'])){?>
        ...
        <?php }?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/payment_modes/edit/'+aData['id']+'">Edit</a>';
          

        <?php if( (in_array('s',$roleResponsible['payment_modes'])) && (in_array('e',$roleResponsible['payment_modes'])) ){?>
        
          $(nRow).find('td:eq(9)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['payment_modes'])) && (in_array('e',$roleResponsible['payment_modes'])) ){?>
           $(nRow).find('td:eq(8)').html(action);
