...
<div class="page-content">
  <div class="page-header-1">
              ...
              <?php if( in_array('a',$roleResponsible['departments'])){?>
                <div class="pull-right ">  
                  <a href="<?php echo base_url();?>admin/department/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add </a>                        
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                      ...
                      <option value="2">Organisation Name</option>
                      <option value="3">Center Name</option>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/department'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/department/all_records'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Department Name", "name":"dept_name","orderable": false, "data":"dept_name", "width":"0%" },            
            <?php if( in_array('s',$roleResponsible['departments'])){?>
              { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},  
            <?php }?>  

            <?php if( in_array('e',$roleResponsible['departments'])){?>
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/department/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/department/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
           <?php if( in_array('s',$roleResponsible['departments'])){?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/department/edit/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
          

        <?php if( (in_array('s',$roleResponsible['departments'])) && (in_array('e',$roleResponsible['departments'])) ){?>
        
          $(nRow).find('td:eq(6)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['departments'])) && (in_array('e',$roleResponsible['departments'])) ){?>
           $(nRow).find('td:eq(5)').html(action);
