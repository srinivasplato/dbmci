...
<div class="page-content">
  <div class="page-header-1">
              ...
              <?php if( in_array('a',$roleResponsible['courses'])){?>
                
                <a href="<?php echo base_url();?>admin/courses/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add </a>                          
                <input type="hidden" name="hiv" id="hiv" value="0" />
              
              <?php }?>
              <a href="<?php echo base_url();?>admin/courses/download_courses" class="btn btn-success btn-sm" type="button"><i class="fa fa-download fa-lg"></i> Download Courses </a>
               </div>
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                      ...
                      <option value="2">Course Type</option>
                  </select>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/courses'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/courses/all_records'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            //{ "title": "Course Type", "name":"course_type","orderable": false, "data":"course_type", "width":"0%" },
        ...
        {           
         ...
         if(aData['course_type'] == 'admin'){
            var course_type1 = 'Admin';
            var course_type2='<p style="color:red">'+course_type1+'</p>';
          }else{
             var course_type1 = 'App';
             var course_type2='<p style="color:green">'+course_type1+'</p>';
          }

          //$(nRow).find('td:eq(5)').html(course_type2);

          if(aData['status'] == 'Active')
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/courses/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/courses/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
           <?php if( in_array('s',$roleResponsible['courses'])){?>
        ...
        <?php }?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/courses/edit/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
          $(nRow).find('td:eq(8)').html(action);
      
         <?php if( (in_array('s',$roleResponsible['courses'])) && (in_array('e',$roleResponsible['courses'])) ){?>
        
          $(nRow).find('td:eq(8)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['courses'])) && (in_array('e',$roleResponsible['courses'])) ){?>
           $(nRow).find('td:eq(7)').html(action);
