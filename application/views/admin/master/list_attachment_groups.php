  ...
  <div class="main-content-inner">
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <ul class="breadcrumb">
          ...
          <li class="active">List Attachment Groups</li>
        </ul><!-- /.breadcrumb -->            
    </div>
...
<div class="page-content">
  <div class="page-header-1">
      ...
      <?php if( in_array('a',$roleResponsible['attachment_groups'])){?>
        <div class="pull-right "> 
          <a href="<?php echo base_url();?>admin/attachment_groups/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add Attachment Groups</a>                          
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">                      
                      <option value="1">Attachment Group Name</option>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/attachment_groups'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
            </table>                                            
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/attachment_groups/all_records'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
           ...
           // { "title": "Center", "name":"center","orderable": false, "data":"center", "width":"0%" },
            { "title": "Attachments  Group Name", "name":"attachment_group_name","orderable": false, "data":"attachment_group_name", "width":"0%" },
            { "title": "Attachments", "name":"attachments","orderable": false, "data":"attachments", "width":"0%" },
            { "title": "Created On", "name":"created_on","orderable": false, "data":"created_on", "width":"0%" },
          <?php if( in_array('s',$roleResponsible['attachment_groups'])){?>
            { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
          <?php }?>

          <?php if( in_array('e',$roleResponsible['attachment_groups'])){?>
        ...
        {           
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/attachment_groups/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/attachment_groups/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
        ...
        <?php }?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/attachment_groups/edit/'+aData['id']+'">Edit</a>';
         
        <?php if( (in_array('s',$roleResponsible['attachment_groups'])) && (in_array('e',$roleResponsible['attachment_groups'])) ){?>
        
          $(nRow).find('td:eq(5)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['attachment_groups'])) && (in_array('e',$roleResponsible['attachment_groups'])) ){?>
           $(nRow).find('td:eq(4)').html(action);
