...
<div class="page-content">
  <div class="page-header-1">
              ...
              <?php if( in_array('a',$roleResponsible['states'])){?>
                <a href="<?php echo base_url();?>admin/states/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add </a> 
              <?php }?>
                <a href="<?php echo base_url();?>admin/states/download_states" class="btn btn-success btn-sm" type="button"><i class="fa fa-download fa-lg"></i> Download States </a>  
        ...
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/states'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/states/all_states'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/states/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/states/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
          $(nRow).find('td:eq(3)').html(action);

      
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/states/edit/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
          

         <?php if( (in_array('s',$roleResponsible['states'])) && (in_array('e',$roleResponsible['states'])) ){?>
        
          $(nRow).find('td:eq(4)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['states'])) && (in_array('e',$roleResponsible['states'])) ){?>
           $(nRow).find('td:eq(3)').html(action);
