      ...
      <h1 class="col-lg-3 col-sm-4 col-md-5 col-xs-7 mbl-mgbtm-5 pdg-top-10">
       ...
       Organisations
        ...
        <div class="pull-right "> 
          <?php if( in_array('a',$roleResponsible['organisations'])){?>
          <a href="<?php echo base_url();?>admin/organisations/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add Organisations</a>                          
          <input type="hidden" name="hiv" id="hiv" value="0"/>
        <?php }?>
         <a href="<?php echo base_url();?>admin/organisations/download_organisations" class="btn btn-success btn-sm" type="button"><i class="fa fa-download fa-lg"></i> Download Organisations </a>  
       </div>  
  </div><!-- /.page-header -->
        ...
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/organisations'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
            </table>                                            
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/organisations/all_organisation'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            <?php if( in_array('s',$roleResponsible['organisations'])){?>
            { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
          <?php }?>
          <?php if( in_array('e',$roleResponsible['organisations'])){?>
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/organisations/change_organisation_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/organisations/change_organisation_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
          <?php if( in_array('s',$roleResponsible['organisations'])){?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/organisations/edit/'+aData['id']+'">Edit</a>';
          
          <?php if( (in_array('s',$roleResponsible['organisations'])) && (in_array('e',$roleResponsible['organisations'])) ){?>
        
          $(nRow).find('td:eq(5)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['organisations'])) && (in_array('e',$roleResponsible['organisations'])) ){?>
           $(nRow).find('td:eq(4)').html(action);
