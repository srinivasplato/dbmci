  ...
  <div class="main-content-inner">
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <ul class="breadcrumb">
          ...
          <li class="active">List Attachments</li>
        </ul><!-- /.breadcrumb -->            
    </div>
...
<div class="page-content">
  <div class="page-header-1">
      ...
      <?php if( in_array('a',$roleResponsible['attachments'])){?>
        <div class="pull-right "> 
          <a href="<?php echo base_url();?>admin/attachments/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add Attachment</a>                          
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">                      
                      <option value="1">Attachment Name</option>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/attachments'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
            </table>                                            
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/attachments/all_records'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
           ...
           // { "title": "Center", "name":"center","orderable": false, "data":"center", "width":"0%" },
            { "title": "Attachments Name", "name":"attachment_name","orderable": false, "data":"attachment_name", "width":"0%" },
          ...
          <?php if( in_array('s',$roleResponsible['attachments'])){?>
            { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
          <?php }?>

          <?php if( in_array('e',$roleResponsible['attachments'])){?>
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/attachments/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/attachments/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
        ...
        <?php }?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/attachments/edit/'+aData['id']+'">Edit</a>';
         
        <?php if( (in_array('s',$roleResponsible['attachments'])) && (in_array('e',$roleResponsible['attachments'])) ){?>
        
          $(nRow).find('td:eq(5)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['attachments'])) && (in_array('e',$roleResponsible['attachments'])) ){?>
           $(nRow).find('td:eq(4)').html(action);
