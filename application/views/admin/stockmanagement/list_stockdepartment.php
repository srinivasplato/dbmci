  ...
  <div class="main-content-inner">
    <div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
              ...
              <li class="active">Stock Names</li>
            </ul><!-- /.breadcrumb -->            
          </div>
...
<div class="page-content">
  <div class="page-header-1">
              <h1 class="col-lg-3 col-sm-4 col-md-5 col-xs-7 mbl-mgbtm-5 pdg-top-10">
               ...
               <?php if( in_array('a',$roleResponsible['in_stock'])){?>
                  <div class="pull-right "> 
                  <a href="<?php echo base_url();?>admin/stockdepartment/add" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus fa-lg"></i> Add </a>                        
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                      ...
                      <option value="5">Center Name</option>
                      <option value="6">Stock Name</option>
                      <option value="7">Count</option>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/stockdepartment'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/stockdepartment/all_records'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Stock Name", "name":"stock_name","orderable": false, "data":"stock_name", "width":"0%" },
            { "title": "Count", "name":"count","orderable": false, "data":"count", "width":"0%" },
             
            <?php if( in_array('a',$roleResponsible['in_stock'])){?>
              { "title": "Status", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
            <?php }?>

           <?php if( in_array('e',$roleResponsible['in_stock'])){?>
        ...
        {           
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/stockdepartment/change_record_status/'+aData['id']+'/Inactive/" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/stockdepartment/change_record_status/'+aData['id']+'/Active/" class="btn btn-danger btn-condensed">Inactive</a>';
          }
          <?php if( in_array('a',$roleResponsible['in_stock'])){?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/stockdepartment/edit/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
          

        <?php if( (in_array('s',$roleResponsible['in_stock'])) && (in_array('e',$roleResponsible['in_stock'])) ){?>
        
          $(nRow).find('td:eq(9)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['in_stock'])) && (in_array('e',$roleResponsible['in_stock'])) ){?>
           $(nRow).find('td:eq(8)').html(action);
