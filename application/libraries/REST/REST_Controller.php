...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->request->format && $this->request->body)
        {
            ...
            //print_r($this->{'_' . $this->request->method . '_args'}) ;
        }
    ...
    protected function _detect_api_key()
    {
        ...
        //echo isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name);
        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            echo ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->get($this->config->item('rest_keys_table'))->row());
            if (!($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
            //echo $this->config->item('rest_key_column');
            $this->rest->key = $row->{$this->config->item('rest_key_column')};
            //echo  "Key = ".$this->rest->key;
    ...
    protected function _check_login($username = NULL, $password = FALSE){
        ...
        //echo $username." Now Password ".$password;
        $auth_source = strtolower($this->config->item('auth_source'));
        //echo $auth_source;
        $rest_auth = strtolower($this->config->item('rest_auth'));
        //echo $rest_auth;
		$auther=$this->db->query('SELECT `username`, `hash` FROM service_credentials WHERE `id`=1')->row_array();
        //$valid_logins = $this->config->item('rest_valid_logins');
        $valid_logins =$auther;
        //echo isset($valid_logins[$username]) ? $valid_logins[$username] : '';
        //print_r($valid_logins);exit;
        //echo !($this->config->item('auth_source'));
        if (!$this->config->item('auth_source') && $rest_auth === 'digest')
        {
           // echo md5($username . ':' . $this->config->item('rest_realm') . ':' . (isset($valid_logins[$username]) ? $valid_logins[$username] : ''));
           // echo $username . ':' . $this->config->item('rest_realm') . ':' . (isset($valid_logins[$username]) ? $valid_logins[$username] : '');
            // For digest we do not have a password passed as argument
            //echo isset($valid_logins[$username]) ? $valid_logins[$username] : '';
            //echo $username. ':'. "pw is ".isset($valid_logins[$username]) ? $valid_logins[$username]:'';
        ...
        $query = $this->db->query("select * FROM service_credentials where username='$username'");
        $user = $query->result_array();
        //print_r($user);
        // if($user==NULL)
	    //echo $password.'_'.$user[0]['hash'];exit;
		if(count($user)>0){
			if($user[0]['username'] == $username && $password == $user[0]['hash']){
				return TRUE;
			}
			else{
				//echo "not authorized";
   ...
   protected function _prepare_mb_auth()
    {
       //echo 'Unauthorized';
        // If whitelist is enabled it has the first chance to kick them out
        if ($this->config->item('rest_ip_whitelist_enabled'))
        ...
        $username = $this->input->server('PHP_AUTH_USER');
        //echo $username;exit;
		//$http_auth = isset($_POST['auth_key'])?$_POST['auth_key']:'';
        //print_r($http_auth);
        //$valid_logins = $this->config->item('rest_valid_logins');
		$valid_logins=$this->db->query("SELECT  `username`, `hash` FROM service_credentials WHERE id=1")->row_array();
		$uname =$valid_logins['username'];
		$pwd =$valid_logins['hash'];
		$username =$valid_logins['username'];
		$valid_logins=array($valid_logins['username']=>$valid_logins['hash']);
		$password =  isset($valid_logins[$username]) ? $valid_logins[$username] : '';
        $valid_hash = md5($username.':'.'REST API'.':'.$password);
        // Returns NULL if the SERVER variables PHP_AUTH_USER and HTTP_AUTHENTICATION don't exist
        $username = $this->input->server('PHP_AUTH_USER');
        $http_auth = $this->input->server('HTTP_AUTHENTICATION');
        $username=$uname;
        ...
        //echo 'user ='.$username.'pw = '.$password.'hash = '.$hash;
        if(@$hash === $valid_hash){
        ...
        // Check if the user is logged into the system
        if ($this->_check_login($username, @$hash) === FALSE)
        {
            //echo 'hello';
            $this->_force_login();
        }
    ...
    protected function _prepare_digest_auth()
    {
       ...
       // echo $this->input->server('PHP_AUTH_DIGEST');
        echo $digest_string;
        ...
        //md5(strtoupper($this->request->method));
        //echo strtoupper($this->request->method) . ':' . $digest['uri'];
        //echo "HA2 IS = ".md5(strtoupper($this->request->method) . ':' . $digest['uri']);
        //echo $digest['qop'];
        $valid_response = md5($username.':'.$digest['nonce'].':'.$digest['nc'].':'.$digest['cnonce'] .':'.$digest['qop'].':'.$md5);
        echo $valid_response."       ";
        //$valid_response = md5($username . ':' . $digest['nonce'] . ':' . $md5);
        //echo "Valid Response is = ".$valid_response;
        // Check if the string don't compare (case-insensitive)
        echo $digest['response'].'  :  ' .$valid_response;
